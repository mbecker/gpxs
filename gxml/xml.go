...
func ToXML(g *geo.GPX, params ToXmlParams) ([]byte, error) {
	version := g.Version
...
func ParseBytes(bytes []byte, algorithm geo.Algorithm) (*geo.GPX, error) {
	...
	if version == "1.0" {
		...
		return convertFromGpx10Models(g, algorithm), nil
	} else if version == "1.1" {
		g := &GPX11Gpx{}
		err := xml.Unmarshal(bytes, &g)
		if err != nil {
		...
		return convertFromGpx11Models(g, algorithm), nil
	} else {
...
func ParseString(str string, algorithm geo.Algorithm) (*geo.GPX, error) {
	return ParseBytes([]byte(str), algorithm)
}
